...
def point_in_convex_polygon(point):
    ...
    if point_on_segment(Point(0, 0), seq[0], point) or point_on_segment(seq[n - 1], Point(0, 0), point):
        return "BOUNDARY"

    if point_on_segment(seq[6-1], seq[6], point):
         return "BOUNDARY"
    if point_on_segment(seq[5-1], seq[5], point):
         return "BOUNDARY"
    if point_on_segment(seq[4-1], seq[4], point):
        return "BOUNDARY"
    if point_on_segment(seq[3-1], seq[3], point):
        return "BOUNDARY"
    if point_on_segment(seq[2-1], seq[2], point):
        return "BOUNDARY"
    if point_on_segment(seq[1-1], seq[1], point):
        return "BOUNDARY"

    if sgn(seq[0].cross(point)) != sgn(seq[0].cross(seq[n - 1])) or sgn(seq[n - 1].cross(point)) != sgn(seq[n - 1].cross(seq[0])):
        return "OUTSIDE"
